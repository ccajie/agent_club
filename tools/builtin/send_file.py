        ...
        if as_type == "image":
            return ToolResponse(
                content=[
                    ...
                    TextBlock(type="text", text=f"Image file sent: {os.path.basename(file_path)}"),
                ],
            )
        if as_type == "audio":
            return ToolResponse(
                content=[
                    ...
                    TextBlock(type="text", text=f"Audio file sent: {os.path.basename(file_path)}"),
                ],
            )
        if as_type == "video":
            return ToolResponse(
                content=[
                    ...
                    TextBlock(type="text", text=f"Video file sent: {os.path.basename(file_path)}"),
                ],
            )

        # For generic files, return as text with file path
        return ToolResponse(
            content=[
                TextBlock(
                    type="text",
                    text=f"File sent: {os.path.basename(file_path)}\nPath: {absolute_path}",
                ),
            ],
        )
